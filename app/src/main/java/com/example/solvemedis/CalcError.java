...
public class CalcError {
    ...
    public CalcError(int oneNumber, int otherNumber, int answer) {
        ...
        this.errorBase = this.ans - this.ansUnitsDigit;
    ...
    private int calcSumFlap100() {
        ...
        if (otherFlap > oneFlap)
            err = this.errorBase + otherFlap;
        else
            err = this.errorBase + oneFlap;
        return err;
    }
    ...
    private int calcAnsSwap100() {
        ...
        int err = 0;
        if (Math.abs(this.ansTensDigit - this.ansUnitsDigit) != 0)
